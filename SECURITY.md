...
Open a [private security advisory](https://github.com/earbona23/entra-privilege-auditor/security/advisories/new)
...
`entra-privilege-auditor` audits Entra ID application registrations for excessive permission and abandonment.
...
| **A finding reported as clean** | An over-privileged app scored as safe. An uncatalogued permission must weigh as *unknown*, never as harmless — a permission nobody has classified is not a permission nobody should worry about. A query that failed, was denied, or was silently truncated must never be presented as an absence of findings. Silence and safety are different results. |
